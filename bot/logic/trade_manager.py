...
async def get_total_balance(client: AsyncClient, config: dict, open_trades: dict) -> float:
    try:
        ...
        # 1. יתרת USDT
        total_nlv += next((float(b['free']) + float(b['locked']) for b in account['balances'] if b['asset'] == 'USDT'), 0.0)
        
        # 2. שווי שוק של נכסים פתוחים
        if open_trades:
            for symbol, details in open_trades.items():
                ticker = await client.get_ticker(symbol=symbol)
                total_nlv += details['quantity'] * float(ticker['lastPrice'])
...
async def open_trade(client: AsyncClient, symbol: str, config: dict):
    try:
        ...
        usdt_balance = next((float(b['free']) for b in (await client.get_account())['balances'] if b['asset'] == 'USDT'), 0.0)
        qty = await round_quantity((usdt_balance * (config['position_size_percent']/100)) / curr_price, lot_size['stepSize'])
...
async def dca(client, symbol, config, current_qty, current_avg, trade_id, tp_id, count):
    try:
        if not config['dry_run']: await client.cancel_order(symbol=symbol, orderId=tp_id)
        ...
        if not config['dry_run']: await client.order_market_buy(symbol=symbol, quantity=dca_qty)
